...
3. `server/providers.js` calls the fixed Anthropic Messages or OpenAI Responses endpoint using Node's `fetch`. It rejects refusals, incomplete output, abnormal completion, and empty output before returning text. It does not retry or silently change provider/model.
