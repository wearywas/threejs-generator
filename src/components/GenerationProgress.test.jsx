...
describe('honest generation progress', () => {
  it('shows elapsed time, real attempts and repair billing without percentages or streaming claims', () => {
    ...
    expect(html).toContain('Request attempt 1 of up to 3')
    expect(html).toContain('Repair attempts: 0')
  ...
  it.each([
    ...
    expect(html).toMatch(/<dialog[^>]*>[\s\S]*Waiting for model repair response[\s\S]*Elapsed 0:04[\s\S]*Repair attempts: 1[\s\S]*<\/dialog>/)
